...
!developmentChains.includes(network.name)
    ...
    it("should create a new token", async function () {
      const tokenId = await snykerContract.getCurrentToken()+1;
    ...
    //   const tokenOwner = await snykerContract.ownerOf(tokenId);
    ...
    it("should get a listed token for a given tokenId", async function () {
      const name = "test-sneaker";
      const description = "this is a test sneaker";
      const price = ethers.utils.parseEther("1");
      const tokenURI = "https://test-sneaker.com";
